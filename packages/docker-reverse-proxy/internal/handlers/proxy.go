...
func (a *APIStore) Proxy(w http.ResponseWriter, req *http.Request) {
	...
	// In development mode with local registry, don't set auth headers
	if os.Getenv("ENVIRONMENT") != "development" {
		req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", token.DockerToken))
	} else {
		// Remove any existing Authorization header for local registry
	...
	// Repository prefix can be configured via environment variable
	repoNamespace := os.Getenv("E2B_REGISTRY_NAMESPACE")
	if repoNamespace == "" {
		repoNamespace = "e2b/custom-envs"
	}
	repoPrefix := fmt.Sprintf("/v2/%s/", repoNamespace)
	...
	// In development mode, don't rewrite URLs - use the local registry as-is
	// In production mode, rewrite URLs from e2b/custom-envs to the GCP namespace
	if os.Getenv("ENVIRONMENT") != "development" {
